...
MIME_TYPE_NUMPY           = "application/numpy-series"
...
class VizierDBClient(object):
  ...
  def __getitem__(self, key: str) -> Any:
    ...
    if artifact.artifact_type == ARTIFACT_TYPE_DATASET:
      ...
      if artifact.mime_type == MIME_TYPE_NUMPY:
        return self.get_numpy(key)
      else:
  ...
  def __setitem__(self, key: Any, updated_data: Any = None) -> Any:
    ...
    if type(updated_data) in primitive_type or type(updated_data) in sequence_type:
      ...
      self.export_function(updated_data, key)
    ...
    elif type(updated_data) == np.ndarray:
      self.save_numpy_series(key, updated_data)
    else:
  ...
  def save_numpy_series(self, key: str, data: np.ndarray) -> None:
    sys.err.write("SAVE NUMPY SERIES\n")
    response = self.vizier_request(
      "create_file",
      has_response=True,
      name=key,
      mime=MIME_TYPE_NUMPY,
      properties={
        "filename": f"{key}.npy"
      },
    )
    ...
    np.save(response["path"], data)

  def get_numpy_series(self, key: str) -> np.ndarray:
    key = key.lower()
    if key not in self.artifacts:
      raise ValueError("unknown numpy series \'{}\'".format(key))
    artifact = self.artifacts[key]
    if artifact.artifact_type != ARTIFACT_TYPE_FILE or artifact.mime_type != MIME_TYPE_NUMPY:

...
abstract class BranchWatcherAPIRoutes
{
  ...
  def route(path: Seq[String], args: Map[String, JsValue]): JsValue =
    path.last match {
      ...
      case "workflowSuggest" => Json.toJson(SuggestCommand(projectId = projectId, branchId = branchId, before = args("before").as[Long], after = args("after").as[Long]):Seq[serialized.PackageDescription])

...
class Editor(
    ...
    {
    // This function should take datasetRows.now and translate it into a
    // CommandArgument set that can be sent back to the server.
    ???
    }
...
object SideMenu {
    ...
    def toggleMenu(buttonEvent: dom.MouseEvent, barChart: ChartRow): Unit = {
        if (isOpen) {
        hide()
        isOpen = false
        } else {
    ...
    def renderSideMenuContent(barChart: ChartRow): Unit = {

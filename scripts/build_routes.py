...
WEBSOCKET_API_FILE = "vizier/shared/src/info/vizierdb/api/websocket/BranchWatcherAPI.scala"
...
with open(WEBSOCKET_API_FILE, "w") as f:
  ...
  print("import org.mimirdb.api.request.DataContainer")
  print("")
  print("trait BranchWatcherAPI")
  print("{")

  # hardcoded routes
  print("  def subscribe(projectId: Identifier, branchId: Identifier): serialized.WorkflowDescription")
  print("  def ping(): Long")
  ...
  for idx, path, verb, handler, group, action, return_type, body_fields in routes:
    ...
    fieldString = ", ".join([
      ...
      if fieldName not in WEBSOCKET_IGNORE_ARGS
    ])
...
with open(WEBSOCKET_IMPL_FILE, "w") as f:
  ...
  print("import org.mimirdb.api.request.DataContainer")
  print("")
  print("abstract class BranchWatcherAPIRoutes extends BranchWatcherAPI")
  print("{")
  print("")
  print("  implicit def liftToOption[T](x: T): Option[T] = Some(x)")
  print("  def wrapCall[T](action: String, op: => T): T")
  ...
  for idx, path, verb, handler, group, action, return_type, body_fields in routes:
    ...
    print(f"  def {action}({fieldString}): {return_type} =")
    print(f"    wrapCall(\"{action}\", {handler}({callString}))")
  print("}")
sys.stdout = original_stdout

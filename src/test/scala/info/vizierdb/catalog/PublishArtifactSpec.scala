  ...
  "Publish" >> {
    ...
    project.append("data", "unload")(
      ...
      UnloadDataset.PARAM_OPTIONS -> Seq(
        Map(
          UnloadDataset.PARAM_OPTION_KEY -> "name",
          UnloadDataset.PARAM_OPTION_VALUE -> EXPORT_NAME,
        )
      )
    )
    ...
    GetPublishedArtifactHandler
      .handle(
        Map("name" -> JsString(EXPORT_NAME)), 
        new handler.ClientConnection { 
          def getInputStream = ???
          def getPart(name: String) = ???
          def getParameter(name: String) = null
        }
      ) must beAnInstanceOf[response.RawJsonResponse]

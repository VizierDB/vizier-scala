...
object Geotag extends Command
{
  ...
  def process(arguments: Arguments, context: ExecutionContext): Unit = 
  {
    ...
    val ds = context.dataset(datasetName)
                    .getOrElse { 
    ...
    val schema = MimirAPI.catalog.get(ds).schema
    val lat: StructField = schema.fields(arguments.get[Int](PARAM_LATITUDE))
    val lon: StructField = schema.fields(arguments.get[Int](PARAM_LONGITUDE))
